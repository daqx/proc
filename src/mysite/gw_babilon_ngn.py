...
babilon_ngn = OpService.objects.get(code = 'babilon_ngn')
while True:
    try :
        gatew = Gateway.objects.get(code = 'babilon_ngn')
        ...
        if st_gatew == 'WORKING':
            ...
            if st_route == 'self':
                tr=Transaction.objects.filter(Q(try_count__isnull=True)|Q(try_count__lte=10), Q(state = st_new)|Q(state = st_no_money) , opservices = babilon_ngn).order_by("state")[:3] #лимит по 3
                ...
                if gatew.route.status.code == 'WORKING':
                    ...
                    for i in tr: #цыкл только по тем транзакциям у которых статус =0
                        ...
                        if saldo + d.overdraft - d.limit >= i.summa_pay:     #проверяем баланс диллера
                            ...
                            i.set_state('1023') #не достаточно средств
        time.sleep(3)
        print ('Informations are select from transactions babilon_ngn')
