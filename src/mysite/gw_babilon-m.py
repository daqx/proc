...
#    urllib2.install_opener(opener)
    cert = 'C:/cer_babilonm.cer'
    urllib2.install_opener(cert)
    #открытие соединения
    try:
...
#            return 0
        ...
        err_code, dt_oper, reciept_num = babilon_send_data(trans, url, gatew) #отправляем провайдеру    
...
try :
    while True:
        ...
        if gatew.status.code == 'WORKING':
            st_new=State.objects.get(code='0')  #статус на отправку
            tr=Transaction.objects.filter(Q(try_count__isnull=True)|Q(try_count__lte=10), state = st_new, opservices__code = 'BABILON-M')[:3] #лимит по 3
